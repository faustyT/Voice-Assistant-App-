...
def schedule_reminder(event, event_datetime, recipient_email, meeting_location):
    ...
    def notify():
        ...
        notification.notify(
            title="Reminder!",
            message=f"Upcoming event: {event} in 30 minutes at {meeting_location}.",
            timeout=10
        )
    ...
    st.success(f"Reminder for {event} set at {event_time}. You will be notified 30 minutes before at {meeting_location}.")
...
event_time = st.time_input("Event Time")  # Select time
