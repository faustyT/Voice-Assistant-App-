...
def schedule_reminder(event, event_datetime, recipient_email, meeting_location):
    ...
    send_email("Reminder Notification", f"Upcoming event: {event} at {event_time}", recipient_email)
...
st.markdown("Time Format: `HH:MM AM/PM`  &nbsp; (e.g., `09:00 AM`, `03:45 PM`)")
event_time_str = st.text_input("Event Time (e.g., 03:30 PM)", help="Enter time in 12-hour format with AM or PM (e.g., 09:00 AM or 03:45 PM)")
...
try:
    ...
    event_time_obj = datetime.datetime.strptime(event_time_str, "%I:%M %p").time()

    # Combine date and time into a datetime.datetime object
    event_datetime = datetime.datetime.combine(event_date, event_time_obj)

    # You can now use event_datetime in your reminder
except ValueError:
    st.error("Invalid time format. Please use HH:MM AM/PM (e.g., 03:30 PM)")
