...
event_date = st.date_input("Event Date")  # Select date
st.markdown("Time Format: `HH:MM AM/PM`  &nbsp; (e.g., `09:00 AM`, `03:45 PM`)")
event_time = st.text_input("Event Time", help="Enter time in 12-hour format with AM or PM (e.g., 09:00 AM or 03:45 PM)")
recipient_email = st.text_input("Recipient Email")
meeting_location = st.text_input("Meeting Link or Place (e.g., Zoom/Meet URL or Office Room)")
#try:
 #   event_datetime = datetime.datetime.strptime(f"{event_date} {event_time}", "%Y-%m-%d %I:%M %p")
#except ValueError:
 #   st.error("Please enter time in the correct format: HH:MM AM/PM")
